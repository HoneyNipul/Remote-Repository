...
//const std::string saveFilePath = "../../../../SaveFiles"; // default staging area for incoming files
//const std::string sendFilePath = "../Storage"; // default staging area for outgoing files "../Storage"
...
bool Sender::sendFile(Message msg)
{
  ...
  std::size_t found = msg.file().find_last_of(".");
  std::string fold = msg.file().substr(0, found);
  std::string extension = msg.file().substr(found + 1);
  std::string fileSpec = path + "/" + msg.file();
  std::cout << "File Spec" << fileSpec;
 // std::string fileSpec = "../../../../RemoteRepo/" +  msg.file();

  std::cout << "\n INSIDE FILE TRANSFER \n";
...
public:
  ...
  */
  bool receiveFile(Message msg)
  {
	...
	std::string path = msg.value("savePath");
	std::cout << "\n path:" << path;
