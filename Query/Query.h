...
namespace NoSqlDb
{
  ...
  public:
	...
	void key(Key key) { keyMatch = key; }
    bool match();

    void name(RegExp regExp) { nameRegExp_ = regExp; }    
    bool matchName();
	void keyM(RegExp regExp) { keyReg_ = regExp; }
	bool matchKey();
	...
	void version(std::string version) { version_ = version; }
	bool matchVersion();
  ...
  bool Conditions<P>::match()
  {
    try
    {
      return matchKey() && matchName() && matchDescription() && matchChildren() && matchDateTimeInterval() && matchVersion();
    }
  ...
  bool Conditions<P>::matchKey()
  {
	  if (keyReg_ == "")
		  return true;
	  std::regex re(keyReg_);
	  return std::regex_search(keyMatch, re);
  }
  /*----< test element metadata for name match >---------------------*/
  ...
  bool Conditions<P>::matchVersion()
  {
	  ...
	  std::string::size_type i = keyMatch.find_last_of(".");
	  std::string::size_type j = keyMatch.length();
	  int v = stoi(keyMatch.substr(i + 1, j));
